...
public class JsonRequest {
    ...
    public void makeAllEquipRequest(
            Context context,
            RequestQueue mQueue,
            final ArrayList<ItemCategory> arrayCategory,
                ...
                try {
                    ...
                    while (parentKeys.hasNext()) {
                        ...
                        if (allEquipment instanceof  JSONObject){
                            JSONObject allObjEquip = (JSONObject) allEquipment;
                            Iterator<String> childKeys = allObjEquip.keys();


                            while (childKeys.hasNext()){
                                String subCategoryName = childKeys.next();
                                if (allObjEquip.get(subCategoryName) instanceof JSONObject){
                                    Toast.makeText(
                                            context,
                                            "Ошибка редактирование прайса." +
                                            " Категории без знаков обозначений",
                                            Toast.LENGTH_SHORT).show();
                                }
                                else {
                                    JSONArray allArraySubCategory =
                                            (JSONArray) allObjEquip.get(subCategoryName);
                                    ArrayList<ItemEquipment> itemEquipment = new ArrayList<>();

                                    for (int j = 0; j < allArraySubCategory.length(); j++){
                                        JSONObject subEquipItem =
                                                (JSONObject) allArraySubCategory.get(j);

                                        String article = subEquipItem.getString("article");
                                        String name = subEquipItem.getString("name");
                                        String description = subEquipItem
                                        ...
                                        itemEquipment.add(new ItemEquipment(
                                            article, name, description, generalKey,  price, image));
                                    }
                                    arraySubCategory.add(new ItemSubCategory
                                            (subCategoryName, itemEquipment));
                                }
                            ...
                            for (int i = 0; i < allArrayEquipment.length(); i++) {
                                ...
                                resultChildrenCategory.add(new ItemEquipment(
                                        article, name, description, generalKey,  price, image));
                            }
                        }
                        arrayCategory.add(new ItemCategory(
                                categoryName, resultChildrenCategory, arraySubCategory));
                    }
                } catch (JSONException e) {
                    e.printStackTrace ();
                }
